...
const guardarRespuestas = async (req, res, next) => {
  try {
    ...
    const { usuarioId } = req.params; // Verificar si el usuario existe
    ...
    const formId = await Formulario.find(); // Convertir IDs en ObjectId
...
const obtenerRespuestasUsuario = async (req, res, next) => {
  try {
    const { id } = req.params; // Buscamos el formulario global (suponemos que existe uno)
    ...
    console.log(respuestaDoc); //console.log(formulario); // Creamos un mapeo: preguntaId -> respuesta (si existe)
